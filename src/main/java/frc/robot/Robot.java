...
public class Robot extends TimedRobot {
  private final TalonFX m_fx = new TalonFX(2, "rio");
  ...
  private final double desiredRotations = 5; 
  ...
  public void teleopPeriodic() {
    ...
    */ 
    System.out.println(m_fx.getRotorPosition().getValueAsDouble() / DRIVE_GEAR_RATIO);
    if (m_joystick.getLeftBumper()) {
      /* Use voltage position */
      m_fx.setControl(m_voltagePosition.withPosition(desiredRotations * DRIVE_GEAR_RATIO));
    }
    else if (m_joystick.getRightBumper()) {
      /* Use torque position */
      m_fx.setControl(m_voltagePosition.withPosition(0));
    }
    else {
